...
async def list_showtimes(db: AsyncSession = Depends(get_db)):
...
async def create_showtime(data: ShowtimeCreate, db: AsyncSession = Depends(get_db)):
    """
...
async def update_showtime(showtime_id: int, data: ShowtimeUpdate, db: AsyncSession = Depends(get_db)):
    service = ShowtimeService(db)
